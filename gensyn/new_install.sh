...
if [[ -n "$NODE_VERSION" && $(echo -e "$NODE_VERSION\n4.0.0" | sort -V | head -n1) == "$NODE_VERSION" ]]; then
    echo "Error: Node.js version is lower than 4.0.0 ($NODE_VERSION). Please upgrade manually."
    exit 1
fi
...
#lets go!
echo "Ставим python dependencies (2-5 мин)..."
